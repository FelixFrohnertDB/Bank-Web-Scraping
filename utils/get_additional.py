...
#Funktion die Handelsregister schreibt
def mainHandelsVr():
        #Für die Url der Entgeltinfos öffnen
        file = open("UrlVrInstitut.txt","r")
        #Für den Tag des Entgeltes öffnen
        file2 = open("HandelsregisterVr.csv","w+")
        cnt = 0
        for num in range(len( open(   "UrlVrInstitut.txt").readlines(    )  ) ):
...
def mainHandelsSp():
        #Für die Url der Entgeltinfos öffnen
        file = open("UrlSpInstitut.txt","r")
        #Für den Tag des Entgeltes öffnen
        file2 = open("HandelsregisterSp.csv","w+")
        cnt = 0
        for num in range(len( open(   "UrlSpInstitut.txt").readlines(    )  ) ):
            url = file.readline().replace('\n','')
            if url[:5] == "https":
                name = url[12:-3].replace(".","")
            else:
...
def findTagSp():
    ...
    file2 = open("UrlVrTag.csv","w+")
    for url in range(len( open(   "UrlVrFiliale.txt").readlines(    )  ) ):
        try:
...
#Test
def get_data_sp2(url,proxy=None):
...
def mainTAG(proxy=None):
    ...
    for iter in range(lenUrl):
        ...
        data[1]= data[1].replace("https://www.","").replace(".de/","").replace(".de?n=true","").replace("http://www.","").replace("/home.html","").replace(".de","")
        #print(data[1])
        list.append(url)
        #print(list)
        try:
